...
public class MarkdownTaskParser {
    ...
    public EgeTask parseBatch(String rawContent) {
        // Нормализуем переносы строк для совместимости с Windows/Mac/Linux
        String content = rawContent.replace("\r\n", "\n").replace("\r", "\n");

        Matcher yamlMatcher = YAML_PATTERN.matcher(content);
        if (!yamlMatcher.find()) {
            throw new IllegalArgumentException("Ошибка парсинга: не найден блок YAML Frontmatter. Проверьте дефисы ---");
        }
    ...
    private Integer extractInt(String text, Pattern pattern) {
        ...
        return m.find() ? Integer.parseInt(m.group(1).trim()) : 1; // 1 - дефолтное значение
    }
