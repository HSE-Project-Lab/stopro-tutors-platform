  ...
  return (
    <Card className="border-indigo-100 shadow-md bg-gradient-to-b from-white to-indigo-50/30">
      <CardHeader
        ...
        action={<Badge variant={isVariation ? 'info' : 'default'}>{isVariation ? 'Вариант' : 'Прототип'}</Badge>}
      />

      <div className="space-y-5">
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <LaTeX className="text-lg text-slate-900 leading-relaxed">{currentTask.contentLatex}</LaTeX>
        </div>
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-2 font-semibold">
                  ...
                  {isCorrect ? 'Верно! Отличная работа.' : 'Пока неверно, но это нормально — смотрим решение.'}
                </div>
              </div>

              <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-indigo-700 mb-2">Ответ</p>
                <p className="text-lg font-bold text-indigo-900">{currentTask.answer}</p>
              </div>

              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Разбор решения</p>
                <LaTeX className="text-slate-700 leading-relaxed">{prototype.solutionLatex}</LaTeX>
              </div>

              <Button onClick={handleSolveSimilar} size="lg" className="w-full bg-fuchsia-600 hover:bg-fuchsia-700 focus:ring-fuchsia-500 shadow-fuchsia-200">
